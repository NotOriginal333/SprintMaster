...
import {
    ...
    TextField, MenuItem, DialogActions, LinearProgress, IconButton, Tooltip
} from '@mui/material';
import {
    BarChart as ChartIcon, Add as AddIcon, CheckCircle, HourglassEmpty, Visibility
...
export const ReportsPage = () => {
    ...
    // Polling: Оновлюємо кожні 3 сек, якщо є незавершені звіти
    useEffect(() => {
        ...
        <Box>
            ...
            <ReportsTableContainer elevation={1}>
                <Table>
                    ...
                    <TableBody>
                        {reports.length === 0 ? (
                            <TableRow><TableCell colSpan={6} align="center">Звітів поки немає</TableCell></TableRow>
                        ) : (
                            reports.map((row) => (
                                <TableRow key={row.id} hover>
                                    ...
                                    <TableCell>
                                        {row.is_ready ? renderReportData(row.data) : (
                                            <Box display="flex" alignItems="center" gap={1}>
                                                ...
                                                <Typography variant="caption" color="text.secondary">Celery
                                                    працює</Typography>
                                            </Box>
                                        )}
                                    </TableCell>
                                    <TableCell align="center">
                                        {row.is_ready && (
                                            <Tooltip title="Переглянути графіки">
                                                <IconButton onClick={() => setSelectedReport(row)} color="primary">
                                                    <Visibility/>
                                                </IconButton>
                                            </Tooltip>
                                        )}
            ...
            <Dialog open={open} onClose={() => setOpen(false)}>
                ...
                <DialogContent sx={{minWidth: 400, pt: 2}}>
                    <TextField
                        select label="Виберіть проєкт" fullWidth margin="normal"
                    ...
                    <TextField
                        select label="Тип звіту" fullWidth margin="normal"
